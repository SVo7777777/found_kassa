...
root = Tk()
root.title("фазенда")
root.geometry("1500x580+5+5")#1300x460+200+2
root['bg'] = 'orange'
...
main_menu['fg'] = 'green'
file_menu = Menu()
file_menu.add_command(label="open_tabel", command=rew_tabl)
file_menu.add_command(label="open_tabel2", command=total_che)
...
notebook.add(frame1, text="<ТАБЕЛЬ РАБОТНИКОВ>", compound=LEFT)# image=python_logo,
...
def save(s):
    with open('kalkulator00.txt', 'a+') as k:
        k.write('\n' + str(s))
...
for i in range(11):
    for j in range(35):
        if j == 0:
            ...
            if j == 1:
                entry0[i, j].config(width=15, justif='left')
            if j == 33:
...
def summer(en):
    ...
    try:
        ...
        entry0[ind, 34].delete('0', END)
        #b_zp.config(command=lambda: entry0[ind, 34].insert(0, f'{sum_hour[ind]*200}'))
        entry0[ind, 34].insert(0, f'{sum_hour[ind]*200}')
        print(str(en)[28:29])
    except ValueError:
        print('это не число')
def focus1(t):
...
entry0[0, 1].config(width=16, justif='center')
entry0[0, 33].delete('0', END)
entry0[0, 33].insert(0, 'всего')
entry0[0, 33].config(width=6)
entry0[0, 34].delete('0', END)
entry0[0, 34].insert(0, 'з/п')
but0[0, 0].config(text='№№')
def change_color():
    for i in range(1, 11):
        entry0[i, 34].config(fg='red', font=('Arial', 16, 'bold'))
b_zp = Button(canvas_foto2, width=10, bg='lightblue', fg='blue', justif="left", bd=0,
              command=change_color, font=('Arial', 14, 'bold'), text='з/п')#+ '/' + '%s' % j)
b_zp.place(x=35, y=110)
tabl_full = False
def poisk(m, file):
                    ...
                    if m != '':
                        ...
                        if name_sovp or name_sovp1:
                            ...
                            if t <= 16:
                                ...
                                entry0[t, 34].delete('0', END)
                                entry0[t, 34].insert(0, f'{sum_hour[t] * 200}')
...
l_search = Label(canvas_foto, text='Выберите месяц:', bg ='lightblue', fg='blue', font=('Arial', 18, 'bold'))
l_search.place(x=1, y=50)#y=150
...
def uptabl():
    ...
    for i in range(1, 11):
        entry0[i, 34].config(fg='white', font=('Arial', 16, 'bold'))
    with open('tabel.txt', 'r') as f:
        l = f.read()
        lst = []
        lst2 = []
...
def rashod():
    ...
    try:
        ...
        messagebox.showinfo('внимание!', 'действие выполнить невозможно')

save_tablb = Button(canvas_foto, width=18, text='сохранить табель ', bg='lightblue',fg='blue', bd=0 ,font=('Arial', 16, 'bold'))
save_tablb.place(x=800, y=100)
uptablb = Button(canvas_foto, width=18, text='очистить табель',bg='lightblue', command=lambda: (uptabl(), delbut()), fg='blue', bd=0, font=('Arial', 16, 'bold'))
uptablb.place(x=800, y=150)
rewtablb = Button(canvas_foto2, width=10, text='расход', bg='lightblue', command=lambda: rashod(), fg='blue', bd=0, font=('Arial', 16, 'bold'))
rewtablb.place(x=30, y=150)
...
vvod_search.focus()
vvod_search2 = Entry(canvas_foto,  width=10, fg='black', font=('Arial', 20, 'bold'), textvariable=entry_text2)
vvod_search2.place(x=220, y=50)
vvod_search2.bind('<KeyRelease>', check_input2)
...
listbox2 = Listbox(canvas_foto, listvariable=listbox2_values, width=20, fg='black', font=('Arial', 14, 'bold'))
listbox2.bind('<<ListboxSelect>>', on_change_selection2)
listbox2_values.set(lst3)
vivod_all = Button(canvas_foto, command=lambda: poisk('', 'tabel.txt'), text='Показать табель', bd=0, bg='lightblue', fg='blue', font=('Arial', 18, 'bold'))
#vivod_all.place(x=1, y=140)
root.mainloop()
