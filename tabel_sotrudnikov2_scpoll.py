...
root = Tk()
root.title("фазенда")
root.geometry("1500x480+5+5")#1300x460+200+2
root['bg'] = 'orange'
...
canvas8 = Canvas(frame1, width=300, height=190, borderwidth=0, background="green")
canvas8.pack(side="top", fill="both", expand=False)
ph2 = PhotoImage(file='fazenda2.png')
canvas8.create_image(700, 200, image=ph2)
...
for i in range(11):
    for j in range(35):
        if j == 0:
            but0[(i, j)] = Button(scrolled_frame8, width=3, bg='LightSteelBlue', fg='Black', justif="left",
...
def summer(en):
    ...
    try:
        ...
        print(e)
        print(sum_hour)
        ind = int(str(en)[35:36])#имя виджета:.!notebook.!frame.!canvas3.!frame.e225
        print(ind)
...
b_zp = Button(canvas8, width=10, bg='lightblue', fg='blue', justif="left", bd=0,
              command=change_color, font=('Arial', 14, 'bold'), text='з/п')#+ '/' + '%s' % j)
b_zp.place(x=1160, y=150)
tabl_full = False
def poisk(m, file):
...
l_search = Label(canvas8, text='Выберите месяц:', bg ='lightblue', fg='blue', font=('Arial', 18, 'bold'))
l_search.place(x=100, y=10)#y=150
...
save_tablb = Button(canvas8, width=18, text='сохранить табель ', bg='lightblue',fg='blue', bd=0 ,font=('Arial', 16, 'bold'))
save_tablb.place(x=800, y=100)
uptablb = Button(canvas8, width=18, text='очистить табель', bg='lightblue', command=lambda: uptabl(), fg='blue', bd=0 ,font=('Arial', 16, 'bold'))
uptablb.place(x=800, y=150)
rewtablb = Button(canvas8, width=10, text='расход',bg='lightblue', command=lambda: rashod(), fg='blue', bd=0 ,font=('Arial', 16, 'bold'))
rewtablb.place(x=1160, y=100)
...
vvod_search2 = Entry(canvas8,  width=10, fg='black', font=('Arial', 20, 'bold'), textvariable=entry_text2)
