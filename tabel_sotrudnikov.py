...
def menubat0(i, j, sp):
    ...
    if i >= 0:
        for j2 in range(len(sp) - 1):
            ...
            #options[i + 1, j2 + 2].add_command(label=f'изменить за {j2+1} число: ' + a[i+1, j2+2][0], command=lambda: change_name(a[i+1, j2+2][0], a[i+1, j2+2][1], a[i+1, j2+2][2]))
            #options.add_command(label=f'изменить за {j2+1} число: ' + a[i+1, j2+2][0], command=lambda: change_name(a[i+1, j2+2][0], a[i+1, j2+2][1], a[i+1, j2+2][2]))

        #options[i + 1, i+len(sp)].add_command(label=f'ПРОДОЛЖИТЬ за {i+len(sp)} число:', command=lambda: vnos(i+len(sp)))
        options.add_command(label=f'ВНЕСТИ за {i+len(sp)} число:', command=lambda: vnos(i+1, len(sp), sp))
    print(a)
...
def change_n1(i,j,old):
    ...
    with open('tabel.txt', 'r') as file1, open('tabel1.txt', 'w') as file:
    ...
    try:
        ...
        '''if sp == []:
            for k in range(len(sp)):
                entry0[i-1, k+1].bind("<Return>", lambda e: (inputt(i-1, k+1, sp), entry0[i, k+1].focus()))
        else:'''
        entry0[i-1, j+1].bind("<Return>", lambda e: (inputt(i-1, j+1, sp), entry0[i-1, j+2].focus()))
...
for i in range(18):
    for j in range(34):
        if j == 0:
            ...
            entry0[(i, j)] = Entry(canvas_search, width=2, fg='blue',  justif='center',
                                  font=('Arial', 16, ''))
