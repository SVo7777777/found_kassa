...
def sum_chekf():
    ...
    if sum_sp == []:
        ...
        if ito > 10000 and ito <= 50000:
            ...
            chek_itogo.config(text=str(int(ito)) + 'p.')
...
def count_chekf():
    for i in range(40):
        if entry[i, 0].get() == '':
            entry[i, 0].insert(END, '1')
    entry[0, 2].focus()
    entry[0, 2].bind("<Return>", lambda e: (chek_stoimost(0)))
...
chek_sd = Button(canvas10, text='подсчёт', fg='lightgreen', width=14, bd=0, command=count_chekf, font=('Arial', 37, 'bold'))
