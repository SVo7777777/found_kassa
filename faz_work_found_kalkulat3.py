...
sum_sp5 = []
#для калькулятора
def kalkulators(frame3, sum_sp, s):
    ...
    chek_cl = Label(canvas10, text='', fg='lightblue', width=14, font=('Arial', 37, 'bold'))
    chek_cl.place(x=10, y=450)
    def total_chek():
        sum = []
        win = Tk()
        win.title("Paзрешение")
    ...
    def chek_up(sum_sp):
        ...
        try:
            ...
            if sum_sp != []:
                ...
                #messagebox.showinfo('внимание', 'sum_sp не пуст\nочищаем')
                for i in range(40):
                    if entry[i, 0].get() != '':
...
kalkulators(frame3, sum_sp3, 'калькулятор №1 ')
kalkulators(frame4, sum_sp4, 'калькулятор №2 ')
kalkulators(frame5, sum_sp5, 'калькулятор №3 ')
