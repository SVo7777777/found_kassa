...
canvas90 = Canvas(frame1, width=200, height=460, borderwidth=0, background="lightblue")
canvas90.pack(side="left", fill="both", expand=True)
canvas9 = Canvas(frame1, width=100, height=460, borderwidth=0, background="lightblue")
...
for i in range(11):
    for j in range(2):
        if j == 0:
            but90[(i, j)] = Button(scrolled_frame80, width=3, bg='LightSteelBlue', fg='Black', justif="left",
                                  font=('Arial', 10, 'bold'), text='%s' % i)  # + '/' + '%s' % j)
            but90[i, j].grid(row=i, column=j)


        else:
            entry90[(i, j)] = Entry(scrolled_frame80, width=2, name='e' + str(i) + str(j), fg='blue', justif='center',
                                   font=('Arial', 16, ''))
...
entry90[0, 1].config(width=16, justif='center')
...
save_tablb = Button(canvas8, width=16, text='сохранить табель ', bg='lightblue',fg='blue', bd=0 ,font=('Arial', 16, 'bold'))
save_tablb.place(x=100, y=50)
uptablb = Button(canvas8, width=16, text='очистить табель', bg='lightblue', command=lambda: uptabl(), fg='blue', bd=0 ,font=('Arial', 16, 'bold'))
uptablb.place(x=100, y=100)
rewtablb = Button(canvas8, width=10, text='расход',bg='lightblue', command=lambda: rashod(), fg='blue', bd=0 ,font=('Arial', 16, 'bold'))
rewtablb.place(x=360, y=50)
b_zp = Button(canvas8, width=10, bg='lightblue', fg='blue', justif="left", bd=0,
              command=change_color, font=('Arial', 14, 'bold'), text='з/п')#+ '/' + '%s' % j)
b_zp.place(x=360, y=100)
