...
def total_buyers():
    ...
    def diagra():
        ...
        num = []
        mon = []
        color = ['green', 'orange', 'red', 'blue', 'purple', 'gold', 'black', 'rose']
            ...
            for i in range(1, len(li)):
                ...
                if mesj == sp1[0][3:5]:
                    ...
                    for j in range(len(kg1)):  # точки графика число и кг.
                        canv.create_oval(50 + num1[j] * 30 + n, 660 - 5 * kg1[j] - m, 50 + num1[j] * 30 + n + 12,
                                         660 - 5 * kg1[j] + 12 - m, fill=cvet, outline=cvet, tags='del')
                        if j > 0:
                            canv.create_line(50 + num1[j - 1] * 30 + 5 + n, 660 - 5 * kg1[j - 1] + 5 - m,
                                             50 + num1[j] * 30 + 5 + n, 660 - 5 * kg1[j] + 5 - m, width=5, fill=cvet,
