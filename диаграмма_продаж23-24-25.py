...
def total_buyers():
    ...
    kn_diagr_money = Button(wind, text='диаграмма продаж', width=16,  bd=1, bg='blue', fg='white',
                font=('Arial', 15, 'bold'))
    kn_diagr_money.place(x=15, y=360)
    l_per = Label(wind, font=("Arial", 12, 'bold'), text='Всего переводов за ', bg='Goldenrod', bd=0, fg='white')
    l_per.place(x=10, y=470)
    l = Label(wind, font=("Arial", 12, 'bold'), text='Всего наличными за ', bg='Goldenrod', bd=0, fg='white')
    l.place(x=10, y=520)
    l2 = Label(wind, font=("Arial", 18, 'bold'), text='Итого за ', bg='Goldenrod', bd=0, fg='white',)
    l2.place(x=10, y=600-30)
    def otdali():
    ...
    def itog_day2(data):
        ...
        print(math.fsum(per))
        print(math.fsum(nal))
        l_per.config(font=("Arial", 12, 'bold'), text='Всего переводов за ' + data + ': ' + str(math.fsum(per)))
        l_per.place(x=10, y=500-30)
        l.config(font=("Arial", 12, 'bold'), text='Всего наличными за ' + data + ': ' + str(math.fsum(nal)))
        l.place(x=10, y=520)
        l2.config(font=("Arial", 18, 'bold'), text='Итого за ' + data + ': ' + str(math.fsum(nal) + math.fsum(per)))
        l2.place(x=10, y=600-30)
    
    def diagra(file_look, z, nai):
        global nadpisi, total, total_del, t2
        root = Tk()
        ...
        t2 = 10
        nadpisi = False
        #print('in diagra t1=', t1)
        canv = Canvas(root, width=400, height=280, bg="white", cursor='pencil')
        can = Canvas(canv, width=1080, height=580, bg="white")
        ...
        canv.place(x=20, y=0)
        can.create_text(400, 640, text='числа в месяце', fill='blue', font=('Arial', 8, 'bold'))
        if nai == 'чел.':
            can.create_text(35, 300, text='к\nо\nл\nи\nч\nе\nс\nт\nв\nо\n \nл\nю\nд\nе\nй', fill='green', font=('Arial', 8, 'bold'))
        else:
            can.create_text(35, 300, text='к\nо\nл\nи\nч\nе\nс\nт\nв\nо\n \nр\nу\nб\nл\nе\nй', fill='green', font=('Arial', 8, 'bold'))
        kg = []
        num = []
        ...
        def diagr(t1, year, mesj, cvet, k, nai, tot_sum):
            global nadpisi, total, t, st, li, g, total_del, t2
            kg1 = []
            num1 = []
            l = {}
            l_num = {}
            ...
            if total == True:
                ...
                can.create_text(165, t, text=mesja1+' - '+str(summer), anchor='w', font=('Arial', 8, 'bold'), fill=cvet, tags='del')
            for i in range(0, len(li)):
                sp1 = li[i].split()
            ...
            t2 = t2 + 23
            can.create_line(120, t2, 160, t2, width=5, fill=cvet, smooth=True, tags='del')
            can.create_text(165, t2, text=mesja+ year+' - '+str(sum(kg1)), anchor='w', font=('Arial', 8, 'bold'), fill=cvet, tags='del')
            print('t1=', t1)
            t1 = t2
            butt = {}
            def but(k, g):
            ...
            if nadpisi == True:
                t = 40
                can.delete('del3')
                can.create_text(700, 650, text='За месяц: ' + str(sum(kg1)),
                                         font=('Arial', 10, 'bold'),
                                         fill='red', tags='del3')
        ...
        def total_di():
            ...
            if  total_del == False:
                ...
                for mo in range(len(mon)):
                    t = t+20
                    diagr(20, mon[mo], color[mo], mo+1, nai, total_summer)
        ...
        #total_diagr.place(x=1140, y=20)

        def clear():
            global total, t, total_del, t2
            can.delete('del')
            can.delete('del2')
            can.delete('del3')
            total = False
        ...
        combobox = ttk.Combobox(root, values=mes2, state="readonly", width=8, font=('Comic', 9, 'bold'))
