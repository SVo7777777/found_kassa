...
with open('ogurci.txt', 'r') as file:
    line = file.read()
    li = line.splitlines()
    #print(li)
    st = li[1].split()
...
def diagr(mesja, cvet):
    ...
    for i in range(1, len(li)):
        sp1 = li[i].split()
        if mesja == sp1[1]:
            kg1.append(int(sp1[5]))
            num1.append(int(sp1[0]))
            #print(kg1)
            #print(num1)
            for j in range(len(kg1)):#точки графика число и кг.
                ...
                l = Label(canv, text=str(num1[j]) + mesja + ' ' + str(kg1[j]) + 'кг.',  fg=cvet, font=('Arial', 8))
                if nadpisi == True:
...
def selected(event):
    ...
    m1 = combobox.get()
    ind = mes2.index(m1)
    diagr(mes[ind], random.choice(color))
